...
# que HAMPATH es NP-Completo. Esto se debe a la aplicación del 
...
class Etapa6(Etapa):
    ...
    def lanzar_etapa(self):
        ...
        # Creamos nuevo panel
        self.panel_6 = ctk.CTkFrame(self.ventana, corner_radius=0)
        self.panel_6.pack(padx=10, pady=10, fill="both", expand=True)

        panel_titulo = ctk.CTkFrame(self.panel_6, fg_color="#6889B1")
        ...
        panel_subtitulo = ctk.CTkFrame(self.panel_6, fg_color="#A4B6CD")
        panel_subtitulo.pack(padx=30, pady=(50,10))

        label = ctk.CTkLabel(panel_subtitulo, text="HAMPATH es NP-Completo", fg_color="#A4B6CD",
                                font=ctk.CTkFont(size=14,weight="bold"))
        label.pack(padx=30, pady=10)

        panel_texto = ctk.CTkFrame(self.panel_6)
        ...
        "y L'= HAMPATH, así que concluimos que HAMPATH es NP-Completo."

        label = label = ctk.CTkLabel(panel_texto, text=texto)
        label.pack(padx=10, pady=10, side = "top")

        panel_botones_6 = ctk.CTkFrame(self.panel_6)
        ...
        if not satisfacible:
            ...
            boton_anterior = ctk.CTkButton(panel_botones_6, text="Anterior",command=lambda:self.gestor_etapas.anterior(5))
            boton_anterior.grid(row=0, column = 0, padx=(10,20), pady=10)

        else:
            boton_anterior = ctk.CTkButton(panel_botones_6, text="Anterior",command=lambda:self.gestor_etapas.anterior(5.3))
            boton_anterior.grid(row=0, column=0,padx=(10,20), pady=10)

        boton_siguiente = ctk.CTkButton(panel_botones_6, text="Fin",command=lambda:self.gestor_etapas.siguiente(7))
